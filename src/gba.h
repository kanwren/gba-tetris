...
// Shortcut for referencing a specific pixel
#define PIXEL(x, y) OFFSET(x, y, WIDTH)
...
#define KEY_UP(key, buttons) ((buttons) & ~(key))
#define KEY_JUST_PRESSED(key, buttons, oldbuttons) (~(buttons) & (oldbuttons) & (key))
#define KEY_JUST_RELEASED(key, buttons, oldbuttons) ((buttons) & ~(oldbuttons) & (key))
